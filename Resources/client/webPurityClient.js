...
function WebPurityClient() {
	...
	var CHECK_METHOD = 'webpurify.live.check';
	var REPLACE_SYMBOL = '*';
	...
	var myClient = {
		...
		filterText : function(messageData) {
			...
			xhr.onload = function() {
				...
				if (this.responseText == 'null' || this.responseText == undefined) {
					Titanium.API.info('filterText: onload: Returning empty set');
		...
		checkText : function(myText) {
			if (!Titanium.Network.online) {
				Ti.App.fireEvent('WP_CHECKED_TEXT', { status:69,
					errorMsg: Msgs.NO_DATA_SERVICE
				});
			}
			var xhr = Ti.Network.createHTTPClient();
			...
			xhr.onload = function() {
				if (this.status >= httpCodes.BAD_REQUEST) {
					handleErrorResp(this.status, 'WP_CHECKED_TEXT');	
					return;
				}
				if (Tools.test4NotFound(this.responseText)) {
					Ti.App.fireEvent('WP_CHECKED_TEXT', { status:89,
						errorMsg: 'Unable complete request at this time -- Apologize for the service failure.'	
					});
					return;
				}
				Titanium.API.info('filterText: onload: Entered - [' + this.responseText + ']');
				if (this.responseText == 'null' || this.responseText == undefined) {
					Titanium.API.info('checkText: onload: Returning empty set');
					Ti.App.fireEvent('WP_CHECKED_TEXT', { result:[], status:0 });
					return;
				}
				if (this.responseText != null) {
					var jsonNodeData = JSON.parse(this.responseText);
					if (jsonNodeData != null) {
						Titanium.API.info('checkText: onload: SUCCESS');
						Ti.App.fireEvent('WP_CHECKED_TEXT', { result:jsonNodeData, status:0 });
					}
					else {
						Ti.App.fireEvent('WP_CHECKED_TEXT', { status:99,
							errorMsg: 'Unable to complete request at this time'	
						});
					}
				}
			...
			var encodedMyText = Titanium.Network.encodeURIComponent(myText);
			var targetURL = webPurityUrl + apiKey;
			targetURL = targetURL + '&method=' + CHECK_METHOD + '&text=' + encodedMyText + '&format=json';
			Titanium.API.info('checkText: REST URL: ' + targetURL);
            xhr.open('GET', targetURL);
			xhr.setRequestHeader('Accept', 'application/json');
            //
            // send HTTP request
            //
            Titanium.API.info('checkText: Sending check request ...');
            xhr.send();	
		}
	};
