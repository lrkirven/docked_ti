...
function BirdHouse(params) {
	...
	function tweet(text,callback) {
		...
		Ti.API.info('tweet(): authorized --> ' + authorized);
		...
		if (authorized === false) {
			authorize(function(resp){
				if (resp) {
					Ti.API.info('tweet(): authorize(): GOOD -- ' + resp);
					obj.tweet(obj.mytweet);

					return true;
				} else {
					Ti.API.info('tweet(): authorize(): BAD -- ' + resp);
					...
					Ti.API.info('tweet(): authorize(): BAD -- Returning false');
					return false;
				}
			});
		} else {
	...
	function send_tweet(params,callback) {
		api('http://api.twitter.com/1/statuses/update.json','POST',params,function(resp){
			Ti.API.info('Got response from Twitter ... ' + resp);
			if (resp!=false) {
				if (typeof(callback)=='function') {
					callback(true);
				}
